...
class AmazonDataset(WILDSDataset):
    ...
    def eval(self, y_pred, y_true, metadata):
        ...
        elif self.split_scheme.endswith('generalization') or self.split_scheme.endswith('baseline') or self.split_scheme in ('category_subpopulation', 'time', 'time_baseline'):
            results_str = (
                f"Average {self._metric.name}: {results[self._metric.agg_metric_field]:.3f}\n"
            )
            for group_idx in range(self._eval_grouper.n_groups):

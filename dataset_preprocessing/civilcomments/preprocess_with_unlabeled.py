...
TRAIN, VAL, TEST, UNLABELED = ('train', 'val', 'test', 'unlabeled')
...
def load_labeled_df(root):
    ...
    """
    df = pd.read_csv(os.path.join(root, 'all_data_with_identities.csv'))
    return df
...
def merge_dfs(unlabeled, labeled):
    ...
    """
    common_cols = [col for col in set(unlabeled.columns).intersection(labeled.columns)]
    unlabeled = unlabeled[common_cols]
    return unlabeled.append(labeled, ignore_index = True)
...
def main(args):
    ...
    assert joint.columns == unlabeled.columns
    ...
    df.to_csv(f'{args.root}/all_data_with_identities_and_unlabeled.csv', index=False)
    print("Done.")

...
def _record_patches(center_size,
    ...
    for i in range(margin, width-margin):
        for j in range(margin, height-margin):
            ...
            # Tumor or normal masks -Tony
            # Just use tissue mask instead.
            ...
            # Compare tissue to normal_threshold
...
def generate_file(patient, node, xml_path, slide_path, folder_path):
    ...
    # We need tissue masks to deal with tissues. don't need tumor mask and normal mask
...
def generate_files(slide_root, output_root):
    ...
    # Generates patches from a big slide
        ...
        for file in files:
            if file.endswith('.xml') and not file.startswith('._'):
                ...
                try:
                    ...
                    # No more xml path and masks
                    df = generate_file(
                        patient=prefix.split('_')[1],
                        node=prefix.split('_')[3],
                        # XML path has the labels but we don't need for Unlabeled Camelyon
